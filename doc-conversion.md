...
- [ ] Delete the existing `docs/reference/*.rst` files. They're regenerated by
  Sphinx each build anyway, so they're not load-bearing.
- [ ] Verify the generated reference pages render correctly — particularly
  `compartment-component` classes with the unusual `__call__` signatures.
- [ ] Watch for **two known pain points** that may need workarounds:
  1. **Namespace packages.** `laser.cholera` is a namespace under `laser`
     (the parent has no `__init__.py`). mkdocstrings handles this via the
     `griffe` discovery options; may need
     `paths: ["src"]` and `options.allow_inspection: false` in `mkdocs.yml`.
  2. **The legacy "Spring likelihood" / "Translation complete" narrative**
     in the `calc_model_likelihood.py` module docstring. mkdocstrings will
     dutifully render it. This is a separate cleanup (see M-series of
     `assessment.md`).
