...
- **Range**: `>= 0` (scalar branch asserts `params.epidemic_threshold >= 0`; array branch asserts `np.all(params.epidemic_threshold >= 0)` **and** `params.epidemic_threshold.shape == (npatches,)`; raises `RuntimeError` if the value is neither a scalar nor an `np.ndarray`). The array form is shape-checked at both ingestion (`dict_to_propertysetex`) and validation time, so a wrong-length array fails at `get_parameters` rather than silently broadcasting inside `np.where`.
